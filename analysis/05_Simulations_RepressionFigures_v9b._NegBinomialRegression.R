...
v9b. Make to match non neg. binom.
	A. Only 3 facets
	B. Update plot style
...
# combine
toPlot <- rbind(toPlot_sf, toPlot_sworld, toPlot_hk)
toPlot <- subset(toPlot, sampleSize >= 1000)  # Will make facet only length 3

### Plot, all
ggplot(toPlot, aes(x=repression_rate_max, y=value, linetype=Significance, color=graph_type)) + geom_rect(aes(xmin=.054, xmax=.1005, ymin=0, ymax=1), fill='grey95', alpha=.05, inherit.aes=FALSE) + geom_line() + facet_wrap(~sampleSize, ncol=3, dir='h') + theme_classic() +  xlab('Repression Rate') + ylab('Percent of Regressions') + theme(axis.text.x = element_text(angle = 90, hjust = 1), axis.text=element_text(size=16), axis.title=element_text(size=16), legend.text=element_text(size=14), legend.title = element_text(size=14), strip.text=element_text(size=16), legend.position='bottom', legend.box='vertical') + labs(color='Graph Type') + scale_color_discrete(limits=c('Watts-Strogatz', 'Holme-Kim', 'Barabasi-Albert')) + scale_linetype_manual(breaks=c('Neg', 'Pos', 'Zero'), values=c('solid', 'dotted', 'longdash'), labels=c('Neg.', 'Pos.', 'Zero'))
ggsave('Figures/August2020/negBinom_compareNetworks_significance_facet3_color.jpg', plot=last_plot())
...
ggsave('Figures/August2020/negBinom_compareNetworks_significance_facet_grey.jpg', plot=last_plot())
